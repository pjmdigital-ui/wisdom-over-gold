...
(async () => {
  ...
  await page.mouse.click(720, 750); // Row3's bar (bottom Section)
  await page.waitForTimeout(1000);
  await page.screenshot({ path: "screenshots/cr3n-00-clicked.png", fullPage: false });
  ...
  console.log("Row3 content: length=" + value.length);
  ...
  fs.writeFileSync("/tmp/row3_check_now.html", value, "utf8");

  await context.close();
})().catch((err) => {
  console.error("CHECK_ROW3_NOW_ERROR", err);
  process.exit(1);
});
